...
const fetchData = async (interval) => {
  try {
    const response = await axios.get(`https://shopifybackend-jv99.onrender.com/api/repeat-customers/?interval=${interval.value}`)
...
const chartData = computed(() => ({
  ...
  datasets: [{
    ...
    data: rawData.value?.map(item => item.repeat_customers) || [],
